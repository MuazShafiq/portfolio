...
- `src/assets/muaz-portrait.jpeg` — the hero portrait source. It's imported and optimized via `astro:assets`' `getImage()` in `index.astro` (WebP re-encode; the raw source is ~250KB but only renders at ~200-300px on screen) and passed into `App` as `portraitSrc`/`portraitWidth`/`portraitHeight` props, since `astro:assets` only runs at the `.astro` build layer and `App.tsx` is a hydrated React island that can't call it directly. If the photo changes, replace this file — don't add a new one under `public/`.
- `public/` — static assets served as-is: résumé PDF, favicon, `site.webmanifest`, PWA/touch icons (`icon-192.png`, `icon-512.png`, `apple-touch-icon.png` — regenerate from `favicon.svg` via `sharp` if the brand mark changes), `robots.txt`, `sitemap.xml`, and a Netlify-style `_headers` file (informational only; actual response headers for the Vercel deployment are defined in `vercel.json`). `muaz-portrait-original.jpeg` also still lives here, referenced only by the `og:image`/`twitter:image`/JSON-LD social meta tags in `index.astro` — it's a portrait-oriented crop and doesn't work well as a landscape social-share card as-is; fixing that needs a deliberate, human-reviewed crop (or a different source photo), not an automated one — see git history for why this was left alone.
...
- Security-related response headers (CSP-adjacent headers, caching for `/_astro/*`, résumé, portrait, and PWA icons) are defined centrally in `vercel.json` — update there, not in `public/_headers`, when changing production headers.
